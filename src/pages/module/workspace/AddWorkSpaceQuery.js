...
const AddWorkSpaceQuery = ({ onClose , finalFunction}) => {
    ...
    const handleOpsUserChange = (e) => {
        ...
        fetchOpsUsers(selectedOpsUser);
    }

    const handleAssignUserChange = (e) => {
        const selectedAsssignUser = $(e.target).val();
    ...
    const handleSubmit = async (e) => {
        ...
        if(!formData.generic_query && !formData.hold_query){
            if(!formData.assignType){
                toast.error("Please select assign user type!");
                return;
            }
        ...
        const fieldsToInclude = {
            ...
            submitBtn: "Submit",
            generic_query: formData.generic_query ? 1 : 0,  // Convert to 1 or 0
            hold_query: formData.hold_query ? 1 : 0  ,
    ...
    return (
        ...
        >
            <div className='flex items-center justify-between py-3 px-2 bg-[#cfe1e5] text-[#02313a]'>
                <h1 className='text-md font-bold'>Add New WorkSpace Query</h1>
            ...
            <div className="flex w-full h-full overflow-y-auto custom-scrollbar items-start justify-center space-x-2 mt-4 ">
                <form encType="multipart/form-data" className="space-y-4 p-4 border-t-2 rounded border-green-400 bg-white shadow-xl w-3/4 qhpage" autoComplete="off">
                    ...
                    <div className="row form-group mt-0 anq">
                        ...
                        <div className="col-sm-4" style={{display: !formData.generic_query ? "block" : "none"}}>
                        ...
                        <div className="col-sm-4" style={{ display: !formData.generic_query && !formData.hold_query ? "block" : "none" }}>
...
export default AddWorkSpaceQuery;

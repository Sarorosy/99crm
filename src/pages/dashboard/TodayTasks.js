...
const TodayTasks = ({ queries , loading, fetchDashboardQueriesForSocket}) => {
    ...
    return (
        <div className="open-tasks-container" style={{ padding: '15px 8px', border: '1px solid #ddd', borderRadius: '6px', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.1)', backgroundColor: '#f0a04b', maxWidth: '350px' }}>
            <h1 style={{ fontSize: '16px', fontWeight: 'bold', color: '#02313a', marginBottom: '10px' }}>Today's Tasks <span className='text-green-600 bg-transparent rouneded-full px-1 py-1'>({queries && queries.length})</span></h1>
                            ...
                            <span className="product-description text-gray-600  block">
                                ...
                                {task.assign_follow_up_date && (
                                    <div className="timerCls">
                                        ...
                                        >
                                            Due since: {new Date(task.assign_follow_up_date * 1000).toLocaleDateString('en-GB', {
                                                day: '2-digit',
                                                month: 'short',
                                                year: 'numeric'
                                            })}

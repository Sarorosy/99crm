...
const MailDetailsTab = ({ mailid, onClose }) => {
    ...
    return (
        <div className={fullscreen ? 'custom-modal ' : 'col-md-4 shadow-lg  bg-white px-2 py-2 relative'}>
            <div className={` ${fullscreen ? "custom-modal-content" : ""} `}>

                <div className="flex items-center justify-start space-x-5 my-1">
                    <button
                        onClick={() => setActiveTab("details")}
                        className={`flex items-center gap-2 px-2 py-1 rounded text-sm transition ${activeTab === "details" ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-700"
                            }`}
                    >
                        <Rows4 size={18} /> Details
                    </button>

                    <button
                        onClick={() => setActiveTab("attachments")}
                        className={`flex items-center gap-2 px-2 py-1 rounded text-sm transition ${activeTab === "attachments" ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-700"
                            }`}
                    >

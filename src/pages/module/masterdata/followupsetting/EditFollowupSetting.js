...
const EditFollowupSetting = ({ onClose, afterSave, settingId }) => {
    ...
    return (
        <motion.div
            ...
            className="fixed top-0 right-0 h-full w-full shadow-lg z-50 overflow-y-auto p-6 bg-gray-100 "
        >
            <button
                onClick={onClose}
            ...
            <form onSubmit={handleSubmit} className="space-y-4 p-4 border-t-2 rounded border-blue-400 bg-white">
                ...
                <div className="text-end">
                    <button
                        type="submit"
                        disabled={posting}
                        className="px-2 py-1 bg-teal-500 text-white font-medium rounded hover:bg-teal-600 transition-colors cremove"
                    >
                        {posting ? 'Saving...' : 'Save'}
                    </button>
                </div>
